...
int PS3mca_verify_card() {
  ...
  /* This is the command get id for memory card (SCPH-1020) or PocketStation
   * (SCPH-4000)*/
  ...
  cmd_card_verification[0] =
      PS3MCA_CMD_FIRST; /* First command for ps3mca protocol*/
  cmd_card_verification[1] =
      PS3MCA_CMD_VERIFY_CARD_TYPE; /* Verify what type of card (PS1 or PS2)*/

  /* Send the message to endpoint with a 5000ms timeout. */
  res = libusb_bulk_transfer(handle, BULK_WRITE_ENDPOINT, cmd_card_verification,
                             sizeof(cmd_card_verification), &numBytes,
                             USB_TIMEOUT);
  if (res == 0) {
    printf("\nType of Memory Card:\n");
/* See on screen what is transmitted for debug purpose*/
  ...
  res = libusb_bulk_transfer(
      handle, BULK_READ_ENDPOINT, response_card_verification,
      sizeof(response_card_verification), &numBytes, USB_TIMEOUT);
  if (0 == res) {
...
/* See on screen what arrive for debug purpose*/
      ...
      if (response_card_verification[0] == RESPONSE_CODE &
          response_card_verification[1] == RESPONSE_PS1_CARD) {
        printf("PS1 Memory Card.\n\n");
      }

      /* Verify if there is a PS2 card*/
      else if (response_card_verification[0] == RESPONSE_CODE &
               response_card_verification[1] == RESPONSE_PS2_CARD) {
        printf("PS2 Memory Card.\nFor the moment isn't in roadmap to support "
               "it (see FAQ).\n\n");
      }
      ...
      fprintf(stderr, "Received %d bytes, expected %d.\n", numBytes,
              sizeof(response_card_verification));
    }
  } else {
...
int PS1_get_id() {
  ...
  cmd_get_id[2] = 0x0a;                 /* 14-4=10=0ah lenght of command. Can be
                                           cmd_get_id[2]=sizeof(cmd_get_id)-4*/
  cmd_get_id[3] =
      0x00; /* memset set all to 0x00, but I prefer to specify it a second
               time ;)*/
  /* this is the real command get id with lenght of 10=0x0a*/
  cmd_get_id[4] =
      PS1CARD_CMD_MEMORY_CARD_ACCESS; /* Memory Card Access, principal command
                                         for any action with any memory card*/
  ...
  cmd_get_id[10] =
      0x00; /* Ask the first two significant digits of the number of frame*/
  cmd_get_id[11] =
      0x00; /* Ask the last two significant digits of the number of frame*/
  cmd_get_id[12] =
      0x00; /* Ask the first two significant digits of the frame size*/
  cmd_get_id[13] =
      0x00; /* Ask the last two significant digits of the frame size*/

  /* Send the message to endpoint with a 5000ms timeout. */
  res = libusb_bulk_transfer(handle, BULK_WRITE_ENDPOINT, cmd_get_id,
                             sizeof(cmd_get_id), &numBytes, USB_TIMEOUT);
  if (res == 0) {
...
/* See on screen what is transmitted for debug purpose*/
  ...
  /* Listen for a message.*/
  /* Wait up to 5 seconds for a message to arrive on endpoint*/
  res = libusb_bulk_transfer(handle, BULK_READ_ENDPOINT, bulk_buffer,
                             sizeof(bulk_buffer), &numBytes, USB_TIMEOUT);
  if (0 == res) {
...
/* See on screen what arrive for debug purpose*/
      ...
      if (bulk_buffer[0] == RESPONSE_CODE &
          bulk_buffer[1] == RESPONSE_STATUS_SUCCES) {
#if DEBUG
        printf("Autentication verified.\n\n");
#endif

        /* Verify if is a memory card (SCPH-1020) or a PocketStation
         * (SCPH-4000)*/
        if (bulk_buffer[6] == PS1CARD_REPLY_MC_ID_1 &
            bulk_buffer[7] == PS1CARD_REPLY_MC_ID_2 &
            bulk_buffer[8] == PS1CARD_REPLY_COMMAND_ACKNOWLEDGE_1 &
            bulk_buffer[9] == PS1CARD_REPLY_COMMAND_ACKNOWLEDGE_2 &
            bulk_buffer[10] == PS1CARD_REPLY_NUMBER_FRAME_1 &
            bulk_buffer[11] == PS1CARD_REPLY_NUMBER_FRAME_2 &
            bulk_buffer[12] == PS1CARD_REPLY_FRAME_SIZE_1 &
            bulk_buffer[13] == PS1CARD_REPLY_FRAME_SIZE_2) {
          printf("This card seems to be a original PS memory card (SCPH-1020), "
                 "(SCPH-1170), (SCPH-119X) or a PocketStation (SCPH-4000).\n");
          printf("If you have a PocketStation (SCPH-4000) try to run the \"x\" "
                 "option.\n\n");
        }
      ...
      else if (bulk_buffer[0] == RESPONSE_CODE &
               bulk_buffer[1] == RESPONSE_WRONG) {
...
int PocketStation_get_id() {
  ...
  cmd_get_id_pkst[2] = 0x05; /* 9-4=5=05h lenght of command. Can be
                                cmd_get_id_pkst[2]=sizeof(cmd_get_id_pkst)-4*/
  cmd_get_id_pkst[3] =
      0x00; /* memset set all to 0x00, but I prefer to specify it a second
               time ;)*/
  ...
  cmd_get_id_pkst[4] =
      PS1CARD_CMD_MEMORY_CARD_ACCESS; /* Memory Card Access, principal command
                                         for any action with any memory card*/
  cmd_get_id_pkst[5] = POCKETSTATION_CMD_GET_ID; /* Send Get ID PocketStation
                                                    Command (ASCII "X")*/
  ...
  /* Send the message to endpoint with a 5000ms timeout. */
  res = libusb_bulk_transfer(handle, BULK_WRITE_ENDPOINT, cmd_get_id_pkst,
                             sizeof(cmd_get_id_pkst), &numBytes, USB_TIMEOUT);
  if (res == 0) {
...
/* See on screen what arrive for debug purpose*/
      ...
      if (bulk_buffer[0] == RESPONSE_CODE &
        ...
        /* If responde appears as a Pocketstation, but can responde in various
         * mode.*/
        printf("%x.\n", bulk_buffer[6]);
        printf("%x.\n", bulk_buffer[7]);
        printf("%x.\n", bulk_buffer[8]);
...
int PocketStation_get_dir_date() {
  ...
  cmd_get_dir_date_pkst[2] =
      0x15;                        /* 25-4=21=15h lenght of command. Can be
                                      cmd_get_dir_date_pkst[2]=sizeof(cmd_get_dir_date_pkst)-4*/
  cmd_get_dir_date_pkst[3] = 0x00; /* memset set all to 0x00, but I prefer to
  ...
  cmd_get_dir_date_pkst[5] =
      POCKETSTATION_CMD_5A; /* Send Get Dir_index, ComFlags, F_SN, Date, and
                               Time*/
  /* according to memset all value are set to 0x00*/

  /* Send the message to endpoint with a 5000ms timeout. */
  res = libusb_bulk_transfer(handle, BULK_WRITE_ENDPOINT, cmd_get_dir_date_pkst,
                             sizeof(cmd_get_dir_date_pkst), &numBytes,
                             USB_TIMEOUT);
  if (res == 0) {
    printf("\nSend POKETSTATION get Dir_index, ComFlags, F_SN, Date, and Time "
           "COMMAND\n");
/* See on screen what is transmitted for debug purpose*/
#if DEBUG
    printf("\n%d bytes transmitted successfully.\n", numBytes);
...
/* See on screen what arrive for debug purpose*/
      ...
      if (bulk_buffer[0] == RESPONSE_CODE &
        ...
        if (bulk_buffer[26] == 0x01) {
          ...
          fprintf(stderr, "error for day of the week, received %x expected a "
                          "value from 01 to 07.\n",
                  bulk_buffer[26]);
        }
        ...
        if (bulk_buffer[18] == 0x01) {
          ...
          fprintf(
              stderr,
              "error for mounth, received %x expected a value from 01 to 12.\n",
              bulk_buffer[18]);
        }
        /* Year (century, year)*/
        ...
        printf("Clock set to:\n");
        printf("%x.%x.%x\n\n", bulk_buffer[23], bulk_buffer[22],
               bulk_buffer[21]); /* Hour.minutes.seconds*/
...
int PS1_read() {
  ...
  FILE *output =
      fopen("Readed_memory_card.mcd",
            "wb"); /* Open and create a binary file output in writing*/
  ...
  for (frame = PS1CARD_MIN_FRAME; frame <= PS1CARD_MAX_FRAME; frame++) {
    ...
    /* This is the write command for memory card (SCPH-1020) or PocketStation
     * (SCPH-4000)*/
    /* first 4 byte are about ps3 memory card adapter protocol*/
    cmd_read[0] = PS3MCA_CMD_FIRST;     /* First command for ps3mca protocol*/
    cmd_read[1] = PS3MCA_CMD_TYPE_LONG; /* PS1 type of command*/
    cmd_read[2] = 0x8c; /* 144-4=140=8ch lenght of command. Can be
                           cmd_read[2]=sizeof(cmd_read)-4*/
    cmd_read[3] =
        0x00; /* memset set all to 0x00, but I prefer to specify it a second
                 time ;)*/
    /* this is the real command read with lenght of 140=0x8c*/
    cmd_read[4] = PS1CARD_CMD_MEMORY_CARD_ACCESS; /* Memory Card Access,
                                                     principal command for any
                                                     action with any memory
                                                     card*/
    ...
    /* Send 0x00 134 times (2 Command Acknowledge + 2 Confirmed Address + 128
     * Data Frame + 1 Checksum + 1 Memory End Byte)*/

    /* Send the message to endpoint with a 5000ms timeout. */
    res = libusb_bulk_transfer(handle, BULK_WRITE_ENDPOINT, cmd_read,
                               sizeof(cmd_read), &numBytes, USB_TIMEOUT);
    if (res == 0) {
/* See on screen what is transmitted for debug purpose*/
#if DEBUG
    ...
    /* Listen for a message.*/
    /* Wait up to 5 seconds for a message to arrive on endpoint*/
    res = libusb_bulk_transfer(handle, BULK_READ_ENDPOINT, ps1_ram_buffer,
                               sizeof(ps1_ram_buffer), &numBytes, USB_TIMEOUT);
    if (0 == res) {
      if (numBytes <= sizeof(ps1_ram_buffer)) {
        /* Verify if PS3mca send status succes code*/
        if (ps1_ram_buffer[0] == RESPONSE_CODE &
            ps1_ram_buffer[1] == RESPONSE_STATUS_SUCCES) {
#if DEBUG
        ...
        /* Verify if PS3mca send status wrong code*/
        else if (ps1_ram_buffer[0] == RESPONSE_CODE &
                 ps1_ram_buffer[1] == RESPONSE_WRONG) {
        ...
        /* This permit to select and save only the received Data Frame
         * (PS1CARD_FRAME_SIZE=128 bytes).*/
        /* First 14 bytes are about PS3mca (4 bytes) and PS1 (10 bytes)
         * protocol.*/
        ...
        for (c = 12; c < 12 + 2 + PS1CARD_FRAME_SIZE;
             c++) /* Loop of msb + lsb + all data bytes*/
        {
          checksum =
              checksum ^
              ps1_ram_buffer[c]; /* Checksum = MSB xor LSB xor all Data bytes*/
        }
        ...
        fprintf(
            stderr,
            "Received %d bytes, expected a maximum of %d bytes on frame %d.\n",
...
int PS1_write() {
  ...
  for (frame = PS1CARD_MIN_FRAME; frame <= PS1CARD_MAX_FRAME; frame++) {
    ...
    cmd_write[2] = 0x8a; /* 142-4=138=8ah lenght of command. Can be
                            cmd_write[2]=sizeof(cmd_write)-4*/
    cmd_write[3] =
        0x00; /* memset set all to 0x00, but I prefer to specify it a second
                 time ;)*/
    /* this is the real command write with lenght of 138=0x8a*/
    cmd_write[4] =
        PS1CARD_CMD_MEMORY_CARD_ACCESS; /* Memory Card Access, principal
                                           command for any action with any
                                           memory card*/
    ...
    fread(&cmd_write[10], 1, PS1CARD_FRAME_SIZE,
          input);          /* Send Data Sector (128 bytes)*/
    cmd_write[139] = 0x00; /* Receive Command Acknowledge 1*/
    cmd_write[140] = 0x00; /* Receive Command Acknowledge 2*/
    cmd_write[141] = 0x00; /* Receive Memory End Byte (47h=Good,
                              4Eh=BadChecksum, FFh=BadSector)*/

    /* Send the message to endpoint with a 5000ms timeout. */
    res = libusb_bulk_transfer(handle, BULK_WRITE_ENDPOINT, cmd_write,
                               sizeof(cmd_write), &numBytes, USB_TIMEOUT);
    if (res == 0) {
/* See on screen what is transmitted for debug purpose*/
#if DEBUG
    ...
    if (0 == res) {
      if (numBytes <= sizeof(ps1_ram_buffer)) {
        ...
        /* Verify Memory End Byte (0x4E=BadChecksum)*/
          ...
          for (c = 8; c < 8 + 2 + PS1CARD_FRAME_SIZE; c++) /* Loop started at
                                                              msb(8) and
                                                              finished at last
                                                              data byte(137)*/
          {
            checksum =
                checksum ^
                cmd_write[c]; /* Checksum = MSB xor LSB xor all Data bytes*/
          }
        ...
        /* Verify Memory End Byte (0xFD=Reject write to Directory Entries of
           currently executed file)*/
        else if (ps1_ram_buffer[141] == POCKETSTATION_REPLY_REJECT_EXECUTED) {
          fprintf(stderr, "WARNING Reject write to Directory Entries of "
                          "currently executed file on frame %d.\n",
                  frame);
          fprintf(
              stderr,
              "aborting for prevent to delete the currently executed file.\n");
        ...
        /* Verify Memory End Byte (0xFE=Reject write to write-protected Broken
           Frame region)*/
        else if (ps1_ram_buffer[141] == POCKETSTATION_REPLY_REJECT_PROTECTED) {
          fprintf(stderr, "WARNING The write-protection is enabled by "
                          "ComFlags.bit10 on frame %d.\n",
                  frame);
    ...
    /* Wait for give time to write, on original card (slower) this time is
     * important.*/
    /* This time can make slower the writing data, if it's possible can be
     * better implement directly the clock.*/
    // sleep(1);
    delay(50);
